...
program test_acceleration_mod_2
  ...
  double precision :: coord(Natoms,3), mass(Natoms), distance(Natoms,Natoms), acc(Natoms,3)
  ...
  call compute_acc(Natoms, coord, mass, distance, epsilon, sigma, acc)
  ...
  ! 5. Verification
  write(*,*) "--- Results ---"
  write(*,*) "Calculated Acc X:", acc(1,1)
  write(*,*) "Expected Acc X  :", expected_acc
  write(*,*) " "

  ! Check X
  if (abs(acc(1,1) - expected_acc) < tol) then
  ...
  if (abs(acc(1,2) - expected_acc) < tol) then
  ...
  ! Check Z
  if (abs(acc(1,3) - expected_acc) < tol) then
